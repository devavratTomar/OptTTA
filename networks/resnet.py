...
class BasicBlock(nn.Module):
    ...
    def forward(self, x: Tensor, feats: bool = False) -> Tensor:
        if feats:
            pre_bn_feats = []
            identity = x

            out = self.conv1(x)
            pre_bn_feats.append(out)
            out = self.bn1(out)
            out = self.relu(out)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, feats: bool=False) -> Tensor:
        ...
        if feats:
            ...
            x, feats = self.layer1(x, True)
            pre_bn_feats += feats
            x = self.layer2(x, True)
            pre_bn_feats += feats
            x = self.layer3(x, True)
            pre_bn_feats += feats
            x = self.layer4(x, True)
            pre_bn_feats += feats
