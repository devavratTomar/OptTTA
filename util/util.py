...
def print_text_on_images(imgs, texts):
    out = []

    for img, text in zip(imgs, texts):
        img = img.permute(1, 2, 0)
        img = 255 * (img.detach().cpu().numpy()*0.5 + 0.5).copy()

        y0, dy = 50, 10
        for i, line in enumerate(text.split('\n')):
            y = y0 + i*dy
            cv2.putText(img, line, (5, y ), cv2.FONT_HERSHEY_PLAIN, 1, (10, 255, 10), thickness=1)
        
        # cv2.putText(img, text, (5, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.1, (255, 10, 10))
