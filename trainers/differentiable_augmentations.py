...
class GaussianBlur(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.conv2d(x, gauss_kernel, padding=self.kernel_size//2)
...
class Gamma(CommonStyle):
    ...
    def forward(self, x):
        ...
        random_gamma = 1.0 + self.bias + torch.abs(self.range) * self.uniform_dist((x.size()[0], self.n_channels, 1, 1), x.device)
        x = x ** (random_gamma)
